...
public class ApiAuthorization extends OncePerRequestFilter {
    ...
    public ApiAuthorization(@Value("${demo.api-token}") String token) {
        if(token==null||token.length()<32)throw new IllegalArgumentException("APP_API_TOKEN must be at least 32 characters");
    ...
    @Override protected void doFilterInternal(HttpServletRequest request,HttpServletResponse response,FilterChain chain)throws ServletException,IOException {
        ...
        if(presented==null||!MessageDigest.isEqual(expected,presented.getBytes(StandardCharsets.UTF_8))){
            response.setStatus(401);response.setContentType("application/json");response.getWriter().write("{\"error\":\"Application credential required\"}");return;
        }
