...
public class AppConfiguration {
    @Bean IntegrityClient integrityClient(@Value("${demo.key-url}") String url,@Value("${demo.writer-token}") String token) {return new IntegrityClient(url,token);}
    @Bean ProcessorClient processorClient(@Value("${demo.processor-url}") String url,@Value("${demo.processor-token}") String token) {return new ProcessorClient(url,token);}
