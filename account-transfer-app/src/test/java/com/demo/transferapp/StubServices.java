...
final class StubServices implements AutoCloseable {
    static final String WRITER = "test-application-writer-credential";
    static final String PROCESSOR = "test-application-processor-credential";
    final ObjectMapper json = new ObjectMapper();
    final Map<UUID, SignedOperation> receipts = new ConcurrentHashMap<>();
    final Map<String, SignedOperation> idempotency = new ConcurrentHashMap<>();
    final Map<UUID, Map<String, Object>> accounts = new ConcurrentHashMap<>();
    final Map<UUID, Map<String, Object>> outcomes = new ConcurrentHashMap<>();
    volatile boolean keysUnavailable, processorUnavailable, unexpectedIssueResponse;
    final HttpServer server;
    final ExecutorService executor = Executors.newVirtualThreadPerTaskExecutor();
    StubServices() throws IOException {
        server = HttpServer.create(new InetSocketAddress("127.0.0.1", 0), 0);
        server.createContext("/", this::handle); server.setExecutor(executor); server.start();
    }
    String url() { return "http://127.0.0.1:" + server.getAddress().getPort(); }
    void reset() { receipts.clear(); idempotency.clear(); accounts.clear(); outcomes.clear(); keysUnavailable = false; processorUnavailable = false; unexpectedIssueResponse = false; }
    ...
    static SignedOperation signed(Operation operation) { return new SignedOperation(operation, "test-key", CanonicalEncoder.hashHex(operation, "test-key"), "00".repeat(32)); }
