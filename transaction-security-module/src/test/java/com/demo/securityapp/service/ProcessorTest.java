...
class ProcessorTest {
    ...
    @Test void debitFailureRollsBackEveryFinancialWriteAndRetryCompletes()throws Exception{
        ...
        processor.configureFault(new FaultRequest(s.operation().id(),0,true));processor.process(s.operation().id());
    ...
    @Test void changedAfterVerificationIsNotProcessed()throws Exception{
        SignedOperation s=operation("TRANSFER",alice,bob,100,null);publish(s);processor.configureFault(new FaultRequest(s.operation().id(),400,false));
    ...
    @Test void reconciliationFindsIssuedOperationWithoutSourceOutbox()throws Exception{
        ...
        when(keys.inventory(0)).thenReturn(new IssuanceInventory(List.of(new IssuanceInventoryItem(1,s)),1));
    ...
    @Test void testHooksAreDisabledUnlessExplicitlyEnabled(){Processor ordinary=new Processor(primary,settlement,keys,audit,json,new MockEnvironment(),delivery);assertThatThrownBy(()->ordinary.configureFault(new FaultRequest(UUID.randomUUID(),100,false))).isInstanceOf(NoSuchElementException.class);}
    @Test void failedBrokerAcceptanceRetainsSourceHint() {
        ...
        var failing=mock(OperationDelivery.class);
    ...
    @Test void issuanceInventoryDetectsDeletionBeforeFirstObservation()throws Exception{
        ...
        when(keys.inventory(0)).thenReturn(new IssuanceInventory(List.of(new IssuanceInventoryItem(1,s)),1));processor.reconcile();
