...
/** Separates ordinary application access from operator-only hold and test endpoints. */
@Component
public class ServiceAuthorization extends OncePerRequestFilter {
    ...
    public ServiceAuthorization(Environment env) {
        app = token(env, "processor.app-token");
        admin = token(env, "processor.admin-token");
        if (MessageDigest.isEqual(app, admin)) {
            throw new IllegalArgumentException("Processor app/admin credentials must differ");
        }
    }

    private byte[] token(Environment env, String name) {
        String value = env.getRequiredProperty(name);
        if (value.length() < 24) {
            throw new IllegalArgumentException("Service credential must have at least 24 characters");
        }
        return ("Bearer " + value).getBytes(StandardCharsets.UTF_8);
    }
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if (!isAdmin && (adminOnly || !MessageDigest.isEqual(supplied, app))) {
            ...
            response.getWriter().write("{\"error\":\"Unauthorized service credential\"}");
            return;
        }
        chain.doFilter(request, response);
    }
