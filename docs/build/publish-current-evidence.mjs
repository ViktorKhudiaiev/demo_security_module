...
if(![2,4,5].includes(args.length))throw new Error('Usage: publish-current-evidence.mjs <verification-dir> <new-verification-filename> OR <verification-dir> <recovery-dir> <live-report> <migration-report> [new-verification-filename]');
const outputName=args.length===2?args[1]:args[4];
if(outputName&&!/^local-verification-\d{4}-\d{2}-\d{2}(?:-final)?\.json$/.test(outputName))throw new Error('Invalid verification output filename');
...
async function read(relative){
 ...
 if(typeof data.passed!=='boolean')throw new Error('Measured report must retain an explicit pass/fail result');
...
const [combined,load,scenarios,recovery,live,migration]=await Promise.all([
 ...
 read(path.join(args[0],'scenarios/report.json')),
 ...(args.length===2?[]:[read(path.join(args[1],'report.json')),read(args[2]),read(args[3])])]);
...
const selected=outputName?{[outputName]:verification}:outputs;
