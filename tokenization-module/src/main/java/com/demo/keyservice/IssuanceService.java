...
public final class IssuanceService {
    ...
    public Inventory inventory(long afterSequence, int limit) {
        if (afterSequence < 0 || limit < 1 || limit > 1000) throw new IllegalArgumentException("Invalid inventory pagination");
        List<InventoryItem> items = jdbc.query("SELECT sequence,signed_json FROM issuance_receipts WHERE sequence>? ORDER BY sequence LIMIT ?",
                (rs, row) -> new InventoryItem(rs.getLong(1), decode(rs.getString(2))), afterSequence, limit);
        return new Inventory(items, items.isEmpty() ? afterSequence : items.getLast().sequence());
    }
